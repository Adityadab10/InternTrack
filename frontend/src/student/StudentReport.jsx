...
const StudentReport = () => {
  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-center items-center min-h-[400px] bg-gradient-to-br from-purple-900/20 via-black/40 to-indigo-900/20 rounded-lg border border-purple-500/30 p-6 sm:p-8">
        ...
        <p className="mt-4 text-purple-300 text-center text-sm sm:text-base">Generating your personal report...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="p-6 bg-gradient-to-br from-red-900/20 via-black/40 to-purple-900/20 rounded-lg border border-red-500/30 shadow-lg sm:p-8">
        <div className="flex items-center mb-4">
          <div className="mr-4 text-red-400 text-3xl sm:text-4xl">⚠️</div>
          <div>
            <h3 className="font-bold text-red-300 mb-1 text-lg sm:text-xl">Report Generation Failed</h3>
            <p className="text-red-200 text-sm sm:text-base">{error}</p>
          </div>
        </div>
        <button 
          onClick={() => window.location.reload()}
          className="mt-2 bg-red-800/50 hover:bg-red-700/50 text-red-200 px-4 py-2 rounded-lg transition-colors duration-300 border border-red-500/30 text-sm sm:text-base"
        >
          Try Again
        </button>
      </div>
  ...
  <div className="relative mb-8">
    ...
    <div className="relative bg-black/60 p-6 rounded-lg border border-purple-500/30">
      <h2 className="text-2xl sm:text-3xl font-bold text-purple-300 mb-2">Your Internship Journey Report</h2>
      <p className="text-purple-200 text-sm sm:text-base">A comprehensive analysis of your progress and potential growth areas</p>
    </div>
  </div>
