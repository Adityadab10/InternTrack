...
const StudentMentor = () => {
  ...
  useEffect(() => {
    if (user?.email && registerUser) {
      registerUser(user.email, 'student');
    }
  }, [user]);

  useEffect(() => {
    const fetchMessages = async () => {
      if (!user?.email || !mentorDetails?.email) return;

      try {
        const response = await axios.get(
          `http://localhost:5001/api/messages/${mentorDetails.email}/${user.email}`,
    ...
    fetchMessages();
  }, [user?.email, mentorDetails?.email]);
  ...
  const allMessages = [...dbMessages, ...messages.filter(msg => 
    !dbMessages.some(dbMsg => dbMsg._id === msg._id)
        ...
        <div className="space-y-6">
          ...
          <div className="bg-black/30 rounded-lg p-6 border border-purple-500/30">
            <div className="flex flex-col sm:flex-row items-center space-x-4 mb-6">
              <div className="w-16 h-16 rounded-full bg-gradient-to-r from-purple-600 to-indigo-600 flex items-center justify-center text-2xl font-bold text-white mb-4 sm:mb-0">
            ...
            <div className="mt-10 bg-purple-950/30 p-4 rounded-lg border border-purple-500/20">
              ...
              <div className="h-60 overflow-y-auto bg-black/20 rounded p-3 space-y-2 text-sm">
                {allMessages.map((msg, index) => {
                  ...
                  return (
                    <div
                      key={msg._id || `${timestamp.getTime()}-${index}`}
                    ...
                    >
                      ...
                      >
                        <p className="font-medium text-xs">
                          {isFromMe ? 'You' : mentorDetails.name}
                        </p>
              ...
              <form
                onSubmit={async (e) => {
                  e.preventDefault();
                  if (!newMessage.trim() || !mentorDetails?.email) return;
  
                  try {
                    // First store in database
                    const messageData = {
                      senderId: user.email,
                      recipientId: mentorDetails.email,
                      content: newMessage,
                      messageType: 'message',
                      timestamp: new Date(),
                    ...
                    // If database storage is successful, send through WebSocket
                    if (response.status === 201 || response.status === 200) {
                      sendMessage(mentorDetails.email, newMessage, {
                        senderRole: 'student',
                        senderName: user?.name || 'Student',
                      ...
                      setDbMessages(prev => [...prev, response.data]);
